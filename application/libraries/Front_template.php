...
class Front_template {
    ...
    protected $brand_name = 'Erlangga Lapak Buku';
    ...
    public function load_view($view, $data = array(), $return = FALSE)
    {
        ...
        {
            $js[] = '<script src="' . assets_url('front/js/' . $js_file) . '"></script>';
        }
        $js = implode('', $js);
        ...
        {
            $css[] = '<link rel="stylesheet" href="' . assets_url('front/css/' . $css_file) . '">';
        }
        $css = implode('', $css);

        $header = $this->_ci->load->view('header_front', array(), TRUE);
        $sidebar = $this->_ci->load->view('sidebar_front', array(), TRUE);
        $footer = $this->_ci->load->view('footer_front', array(), TRUE);
        ...
        return $this->_ci->load->view('front_view', array(
            'title' => $title,
...
/* Location: ./application/libraries/Front.php */