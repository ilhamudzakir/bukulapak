var save_method; //for save method string
          ...
          function add_area_shipping()
          {
            ...
            $('.modal-title').text('Add area_shipping'); // Set Title to Bootstrap modal title
          }

          function edit_area_shipping(id)
          {
            controller_name = $('#controller_name').val();
            save_method = 'update';
            $('#form')[0].reset(); // reset form on modals
            ...
            $.ajax({
              //url : "<?php echo site_url('area_shipping/ajax_edit/')?>/" + id,
          ...
          function save()
          {
            ...
            if(save_method == 'add') 
            {
                //url = "<?php echo site_url('area_shipping/ajax_add')?>";
                url = controller_name + "/ajax_add";
            }
            else
            {
              //url = "<?php echo site_url('area_shipping/ajax_update')?>";
          ...
          function delete_area_shipping(id)
          {
            controller_name = $('#controller_name').val();
            if(confirm('Are you sure delete this data?'))
            {
              // ajax delete data to database
                $.ajax({
                  //url : "<?php echo site_url('area_shipping/ajax_delete')?>/"+id,
 ...
 function add_ongkir()
{
  ...
  $('.form-group').removeClass('has-error'); // clear error class
  $('.help-block').empty(); // clear error string
  $('#modal_ongkir').modal('show'); // show bootstrap modal
  $('.modal-title').text('Upload Data Ongkos Kirim'); // Set Title to Bootstrap modal title
}
...
function upload()
{
    ...
    $.ajax({
        type:"POST",
        url: base_url+"area_shipping/upload/",
        data:data,
